...
def test_load_and_split_data_creates_splits(tmp_path):
    ...
    train, val, holdout = load_and_split_data(raw_path=str(dummy_path), output_dir=tmp_path)

    assert not train.empty and not val.empty and not holdout.empty
    assert train["date"].max() < pd.to_datetime("2020-01-01")
    assert val["date"].min() >= pd.to_datetime("2020-01-01")
...
def test_frequency_encode_counts_values():
    ...
    val = pd.DataFrame({"zipcode": [1000, 3000]})
    train, val, freq_map = frequency_encode(train, val, "zipcode")
    assert train["zipcode_freq"].tolist() == [2, 2, 1]
    assert val["zipcode_freq"].tolist() == [2, 0]
...
def test_target_encode_applies_mapping():
    ...
    val = pd.DataFrame({"city_full": ["A", "B"]})
    train, val, te = target_encode(train, val, "city_full", "price")
    assert "city_full_encoded" in train.columns
    assert val["city_full_encoded"].notnull().all()
...
def test_full_pipeline_integration(tmp_path):
    ...
    train, val, holdout = load_and_split_data(raw_path=str(raw_path), output_dir=tmp_path)

    processed_dir = tmp_path / "processed"
    processed_dir.mkdir(exist_ok=True)

    preprocess_split("train", raw_dir=tmp_path, processed_dir=processed_dir, metros_path=None)
    preprocess_split("val", raw_dir=tmp_path, processed_dir=processed_dir, metros_path=None)
    preprocess_split("holdout", raw_dir=tmp_path, processed_dir=processed_dir, metros_path=None)

    out_train, out_val, out_holdout, freq_map, te = run_feature_engineering(
    ...
    assert "year" in out_train.columns
    assert "year" in out_val.columns
    assert freq_map is None
    assert te is not None
